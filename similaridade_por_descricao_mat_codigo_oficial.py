...
# # <center>Identificação de itens - Materiais </center>
# ---
...
pasta_arquivo = r'\\Fscorp05\monitoramento$\08.Desenvolvimento\10_Similaridade_por_Descricao\MAT\01_TEMPLATE_MAT'
...
if var_codigo == 'COD_SISTEMA':
    ...
    SELECT CODIGO AS CODIGO_SISTEMA, DESCRICAO_CADASTRO, REF_FAB_PADRAO AS REFERENCIA
    FROM TASY.CADASTRO_SEM_DUPLICADOS 
    WHERE HOSPITAL_CORRIGIDO = :1 AND TIPO = 'MAT' AND BLOQUEADO = 'N'
    ...
    # Query
    querystring_cadastro = """ 
    SELECT A.CODIGO AS COD_P12, A.DESCRICAO_CADASTRO, B.REFERENCIA
    FROM TASY.CADASTRO_SEM_DUPLICADOS A 
    LEFT JOIN TASY.BASE_PROTHEUS B ON LPAD(B.CORPORATIVO, 10, 0) = LPAD(A.CODIGO, 10, 0)
    WHERE A.HOSPITAL_CORRIGIDO LIKE '%Hospital P12%' AND A.TIPO = 'MAT' AND B.STATUS = 'Desbloqueado'
    GROUP BY A.CODIGO, A.DESCRICAO_CADASTRO, B.REFERENCIA
    """
...
df_similaridade_tab_propria['similaridade_media_tab_prop'] = np.where(
    ...
    (df_similaridade_tab_propria['similaridade_tab_prop'] * 6 +  
     df_similaridade_tab_propria['peso_primeira_palavra_tab_prop'] * 4) / 10,
    
    # Se a condição for falsa
    (df_similaridade_tab_propria['similaridade_tab_prop'] * 5 + 
     df_similaridade_tab_propria['peso_primeira_palavra_tab_prop'] * 2 + 
     df_similaridade_tab_propria['similaridade_medidas_tab_prop'] * 3) / 10)


# In[ ]:


#similaridade_final_tab_prop
#df_similaridade_tab_propria['similaridade_final_tab_prop'] = df_similaridade_tab_propria.apply(calcular_similaridade_final_tab_propria, axis=1)
...
df_similaridade_cadastro['similaridade_media_cadastro'] = np.where(
    ...
    (df_similaridade_cadastro['similaridade_cadastro'] * 6 +  
     df_similaridade_cadastro['peso_primeira_palavra_cadastro'] * 4) / 10,
    
    # Se qualquer condição for falsa
    (df_similaridade_cadastro['similaridade_cadastro'] * 5 + 
     df_similaridade_cadastro['peso_primeira_palavra_cadastro'] * 2 + 
     df_similaridade_cadastro['similaridade_medidas_cadastro'] * 3) / 10)


# In[ ]:


#similaridade_final_cadastro
#df_similaridade_cadastro['similaridade_final_cadastro'] = df_similaridade_cadastro.apply(calcular_similaridade_final_cadastro, axis=1)
...
if var_codigo == 'COD_SISTEMA' :
    ...
    # Realizar o merge utilizando a coluna DESCRIÇÃO_TRATADA
    df_combinado = pd.merge(df_combinado, base_cadastro[['DESCRICAO_TRATADA','DESCRICAO_CADASTRO', 'COD_P12','REFERENCIA']],
                            left_on='item_cadastro', right_on='DESCRICAO_TRATADA', how='left')
...
caminho_base = r"\\Fscorp05\monitoramento$\08.Desenvolvimento\10_Similaridade_por_Descricao\MAT\03_RESULTADO_MAT"
...
nome_do_arquivo_excel = f"{caminho_base}\\similaridade_por_descricao_MAT_{var_hospital}_{data_atual}.xlsx"
